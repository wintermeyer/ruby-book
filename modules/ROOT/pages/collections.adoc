...
[[more-hash-methods]]
=== More Useful Hash Methods

A handful of Hash methods you will reach for all the time.

*Reading a value safely with `fetch`*:

`hash[key]` returns `nil` if the key is missing. `fetch` raises an
error by default, which is often what you actually want. You can also
pass a default:
...
`merge` returns a new hash. The original is untouched. Values in the
second hash win.
